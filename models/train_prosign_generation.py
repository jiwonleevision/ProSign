...
def setup_run(args):
    ...
    if not args.wandb or args.eval:
        os.environ["WANDB_MODE"] = "disabled"
        return False

    if not args.log_all and not utils.is_main_process():
        os.environ["WANDB_MODE"] = "disabled"
        return False
    ...
    init_kwargs = {
        ...
        "config": {key: value for key, value in vars(args).items() if key != "run"},
    }
    if args.log_all and args.output_dir:
        init_kwargs["group"] = Path(args.output_dir).name

    run = wandb.init(**init_kwargs)
    if not args.log_all and args.output_dir:
